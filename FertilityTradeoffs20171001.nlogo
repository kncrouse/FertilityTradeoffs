...
to earn-funds
  ask humans with [ house = myself and adult? = true ] [
    let money present-cost-to-future-benefit * ( 1 + [current-education] of self )
...
to print-data
  file-print ( word
    ...
    present-cost-to-future-benefit " "
    get-humans-data )
end
...
file-open "FertilityTradeoffs20171001_3"</setup>
  ...
  <experiment name="experiment III" repetitions="1000" runMetricsEveryStep="true">
    <setup>setup
file-open "FertilityTradeoffs20171001_4"</setup>
    <go>go</go>
    <final>print-data</final>
    <steppedValueSet variable="child-mortality-rate" first="0.05" step="0.05" last="1"/>
    <enumeratedValueSet variable="adult-mortality-rate">
