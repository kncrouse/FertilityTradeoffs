...
to setup-patches
  ...
  ;ask patches [ set density density + 0.5 ]
  ;ask patches [ set pcolor scale-color red density 0 1 ]
end
...
to maintain-population-density
  let index 0.1
  while [index < 1.0] [
    let human-count count patches with [ resident != nobody and density < index and density > index - 0.1 ]
    let patch-count count patches with [ density < index and density > index - 0.1 ]
    ask max-n-of ( human-count - patch-count * index ) humans with [ [density] of patch-here < index and [density] of patch-here > index - 0.1 ] [age] [ make-dead ]
  ]
end
...
to find-spouse
  if any? other humans in-radius 10 with [ spouse = nobody and adult? and [household-has-empty-cells] of house ] [
    set spouse one-of other humans in-radius 10 with [ spouse = nobody and adult? and [household-has-empty-cells] of house ]
