...
to maintain-population-density
  ...
  while [index <= 1.0] [
    ...
    if  human-count > patch-count * index [
      ask max-n-of ( human-count -  patch-count * index ) humans with [ [density] of patch-here < (precision (index + 0.1) 1 )  and [density] of patch-here >= precision index 1 ] [age] [ make-dead ]]
    set index precision (index + 0.1) 1
  ]
  ask one-of humans with [ [density] of patch-here = 1.0 ] [ make-dead ]
end
...
to join-household
  if resident = nobody and length bidders > 0 [
    ...
    ask max-one-of households with [ member? self [bidders] of myself ] [ funds ] [ add-cell myself ]
    set bidders []
  ]
end
...
"" ""
PENS
"point" 1.0 0 -16777216 true "" ";plot mean [cost] of patches"
