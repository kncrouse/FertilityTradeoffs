...
to setup-new-house
  hatch-households 1 [
    ...
    set label-color one-of base-colors + random 3 - random 3
...
to go
  ...
  ask n-of (count humans with [ adult? ] * adult-mortality-rate) humans with [ adult? ] [ die ]
  ask n-of (count humans with [ not adult? ] * juvenile-mortality-rate) humans with [ not adult? ] [ die ]
  ask households [ update-household ]
end
...
to-report has-empty-cells report ( count cells with [ count humans-here = 0 ] > 0 ) end
...
to increase-property
  ...
  if cell != nobody [
    let neigh one-of neighbors4 with [ count humans-here = 0 and phouse != [phouse] of myself ]
    if neigh != nobody [
      ask neigh
...
true
false
"" "ask humans [ plotxy current-education child-count]\n"
PENS
...
"" ""
PENS
"default" 1.0 0 -5825686 true "" " plot mean [child-count] of humans"
