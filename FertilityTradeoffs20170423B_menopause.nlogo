...
to initialize-human
  ...
  set hidden? false
  set size 1
  set menopause random 100 - random 100
  set shape "person"
...
to initialize-human-with-parents [ p1 p2 ]
  ...
  set menopause ( [menopause] of p1 + [menopause] of p2 ) / 2
  set menopause menopause - random 2 + random 2
  set mom p1
  set dad p2
end
...
to update-household
  ...
  if mother = nobody and father = nobody and child = nobody [
    ;kick-out-child
    household-make-dead ]
end
...
to earn-funds
  ...
  let money3 ifelse-value ( child != nobody ) [ income-per-education-level * ( 1 + [current-education] of child ) ] [ 0 ]
  set funds funds + money1 + money2 + money3
end
...
to make-child
  ifelse [age] of  mother < [menopause] of mother [
  ...
  [ let children humans with [ mom = [mother] of myself ]
    let divided-money funds / ( count children + 0.0000001)
    ;print ( word divided-money " " [menopause] of mother)
    ;ask children [ if house != nobody [ ask house [ set funds funds + divided-money ]]]
    set funds 0]
end
...
1047
222
Economy: Median Costs over Time
Time
Money
0.0
5.0
...
"total education" 1.0 0 -2674135 true "" "plot median [current-education] of humans with [ adult? ] * cost-per-education-level"
...
"" ""
PENS
"number children" 1.0 0 -5825686 true "" "plot median [child-count] of humans with [ adult? ]"
...
"pen-4" 1.0 0 -6459832 true "" "plot median [menopause] of humans with [ sex = \"female\" ]"
