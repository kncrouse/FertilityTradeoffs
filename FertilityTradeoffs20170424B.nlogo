...
to initialize-human-no-parents
  ...
  set adult? true
  set size 1.0
  set preferred-education random 3
...
to initialize-human-with-parents [ p1 p2 ]
  ...
  set house [house] of p1
  set preferred-education (( [preferred-education] of p1 + [preferred-education] of p2 ) / 2 ) - random 2 + random 2
end
...
to go
  ...
  ask humans with [ not adult? ] [ if random-float 1.0 < child-mortality-rate  [ set dying? true ]]
  ;ask households [ print (word self " " count humans with [ house = myself ])]
  tick
end
...
to update-household
  ...
  if not any? humans with [ adult? ] [ kick-out-child household-make-dead ]
end

to earn-funds
  ask humans with [ house = myself and adult? = true ] [
    let money income-per-education-level * ( 1 + [current-education] of self )
...
to find-spouse
  if count humans with [ house = myself and adult? = true ] = 1 and count humans with [ house = myself and adult? = false ] = 0 [
    ifelse count patches with [ phouse = myself] >= 2 [
      ...
      if other-house != nobody [ add-occupant one-of humans with [ house = other-house and adult? = true ] ]]
    [ increase-property ]]
end

to make-child
  if count humans with [ house = myself and adult? = true ] = 2 and count humans with [ house = myself and adult? = false ] = 0 [
...
to add-occupant [ new-human ]
  ask new-human [ ask house [ household-make-dead ]]
...
to household-make-dead
  if not any? humans with [ house = myself ] [
    ask patches with [ phouse = myself ] [
      set pcolor black
      set phouse nobody ]
