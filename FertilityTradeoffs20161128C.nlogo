...
;  ask patch 0 0 [ set density count patches * 0.25 ]
;  while [[ density ] of patch 0 0 > 1 ] [ diffuse density 0.5 ]
;  ask patches [ set density precision density 1 ]
;  ask patches with [ density = 0 ] [ set density 0.1 ]
end
...
;  let index 0
;  while [index <= 1.0] [
;    let human-count count patches with [ resident != nobody and density < precision (index + 0.1) 1 and density >= precision index 1 ]
;    let patch-count count patches with [ density < precision (index + 0.1) 1  and density >= precision index 1 ]
;    if  human-count > patch-count * index [
;      ask n-of ( human-count -  patch-count * index ) humans with [ [density] of patch-here < (precision (index + 0.1) 1 )  and [density] of patch-here >= precision index 1 ] [ make-dead ]]
;    set index precision (index + 0.1) 1
;  ]
;  if any? humans with [ [density] of patch-here = 1.0 ] [ ask one-of humans with [ [density] of patch-here = 1.0 ] [ make-dead ] ]
end
...
to invest-in-children
  if count parents = 2 [ ask cells with [ resident = nobody ] [ make-human [parents] of myself ]]
...
to find-spouse
  if any? other humans with [ spouse = nobody and adult? and [household-has-empty-cells] of house ] [
    set spouse one-of other humans with [ spouse = nobody and adult? and [household-has-empty-cells] of house ]
...
to make-dead
  ...
  update-density-fertility-graph
  update-education-fertility-graph
  die
...
to update-density-fertility-graph
  set-current-plot "Density vs. Fertility"
  ;plotxy [density] of patch-here child-count
  plot-pen-down
  plot-pen-up
end

to update-education-fertility-graph
  set-current-plot "Education vs. Fertility"
