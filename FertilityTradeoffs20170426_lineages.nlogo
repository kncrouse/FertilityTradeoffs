...
to initialize-human-with-parents [ parents ]
  ...
  ;if (random-float 1.0 < 0.05 ) [ set preferred-education preferred-education - random 2 + random 2 ]
...
to go
  if count humans = 0 or ( stop-at > 0 and ( median [generation] of humans ) >= stop-at ) [ stop ]
...
<experiments>
  ...
  <experiment name="experiment II" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <final>print-data</final>
    <enumeratedValueSet variable="child-mortality-rate">
      <value value="0.5"/>
    </enumeratedValueSet>
    <steppedValueSet variable="adult-mortality-rate" first="0.005" step="0.001" last="0.025"/>
    <steppedValueSet variable="cost-per-education-level" first="5" step="5" last="100"/>
    <enumeratedValueSet variable="income-per-education-level">
      <value value="5"/>
    </enumeratedValueSet>
